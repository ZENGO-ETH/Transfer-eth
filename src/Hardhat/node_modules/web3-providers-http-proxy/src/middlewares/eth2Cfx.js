...
function cfx2Eth(options = defaultOptions) {
  ...
  async function getLogs(req, res, next) {
    ...
    await _updateLogBlockIndex(res.result);
  ...
  async function getTransactionReceipt(req, res, next) {
    ...
    if (txReceipt.logs) {
      for (let i in txReceipt.logs) {
        ...
        let logIndex = await getLogBlockIndex(txReceipt.transactionHash, numToHex(Number(i)), txReceipt.epochNumber);
        logs.push({
  ...
  async function _getEpochReceipts(epochNumber) {
    let block = await _getEpochAsBlock(epochNumber, false);
    let receipts = await _batchGetReceipts(block.transactions);
    return receipts;
  }

  async function _batchGetReceipts(txHashes) {
    const batcher = cfx.BatchRequest();
    for(let hash of txHashes) {
      batcher.add(cfx.cfx.getTransactionReceipt.request(hash));
    }
    const receipts = await batcher.execute();
    return receipts;
  }
  ...
  async function _updateLogBlockIndex(logs) {
    for(let i in logs) {
      const log = logs[i];
      if (log.transactionHash && log.transactionLogIndex) {
        log.logIndex = await getLogBlockIndex(log.transactionHash, log.transactionLogIndex, log.epochNumber);
      }
    }
  }
  ...
  async function getTxBlockIndex(txHash, blockNum) {
    if (TransactionIndexCache[txHash] !== undefined) return TransactionIndexCache[txHash];
  ...
  // txLogIndex should be a hexStr
  async function getLogBlockIndex(txHash, txLogIndex, blockNum) {
    if (typeof txLogIndex === 'number') txLogIndex = numToHex(txLogIndex);
    if (LogIndexCache[txHash] !== undefined) return LogIndexCache[txHash][txLogIndex];
    const receipts = await _getEpochReceipts(blockNum);
    let logIndex = 0;
    for(let receipt of receipts) {
      if (!receipt.transactionHash) continue;
      LogIndexCache[receipt.transactionHash] = {};
      for(let i in receipt.logs) {
        LogIndexCache[receipt.transactionHash][numToHex(Number(i))] = numToHex(logIndex);
        logIndex++;
      }
    }
    if (LogIndexCache[txHash] === undefined) {return '0x0';}
    return LogIndexCache[txHash][txLogIndex];
  }
